...
module.exports = class AbstractWritableStream extends stream.Writable {
     ...
     */
    isWritten(filename) {
        return this._paths.has(filename);
    }
    /**
    ...
    _write(chunk, encoding, callback) {
        ...
        fs.lstat(filename, (err, lstats) => {
            ...
            if (this._filterFn && !this._filterFn(filename, lstats)) {
                // logs only for testing; will be removed before the merge
                console.log(`Do not add file ${filename} in ${this._dest} because it does not pass a filter function`);
                return callback();
            }

            // logs only for testing; will be removed before the merge
            console.log(`Add file ${filename} in ${this._dest} because it is time to do this`);
            this._paths.add(filename);
