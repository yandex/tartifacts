...
module.exports = class TarStream extends AbstractWritableStream {
     ...
     *
     * @param {{path: string, relative: string, base: string, cwd: string, stats: fs.Stats, history: string[], subdir: string}} dir — the directory info.
     ...
     * Keeps original path relative to cwd.
     *
     * @param {{path: string, relative: string, base: string, cwd: string, stats: fs.Stats, history: string[], subdir: string}} file — the directory info.
     * @param {function} callback — call this function when processing is complete.
     */
    addFile(file, callback) {
        const filename = file.history[0];
        const previouslyDeferred = this._map.get(filename);
        const promise = previouslyDeferred ? previouslyDeferred.promise : Promise.resolve();

        promise
            .then(() => {
                const deferred = promiseUtils.defer()
                this._map.set(filename, deferred);

                const readable = fs.createReadStream(filename, { autoClose: true })
                    .on('error', (err) =>  deferred.reject(err) || callback(err))
                    .on('end', () => deferred.resolve() || callback());
