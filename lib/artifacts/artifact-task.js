 ...
 * @returns {{tar: boolean, gzip: boolean, gzipOptions: Object, dotFiles: boolean, emptyFiles: boolean, emptyDirs: boolean, transform: Function}}
 */
...
module.exports = class ArtifactTask {
     ...
     * @param {Function} [settings.transform]       Function for transform files.
     */
    constructor(artifact, settings) {
        ...
        const root = path.resolve(artifact.root || settings.root || cwd);
        const _destDir = artifact.destDir || settings.destDir;
        const destDir = _destDir
            ? path.isAbsolute(_destDir)
                ? _destDir : path.join(root, _destDir)
            : root;
        ...
        const gzip = typeof artifact.gzip === 'object' && artifact.gzip || settings.gzip || artifact.gzip || defaultSettings.gzip;
        ...
        const isInvalidTransform = this._settings.transform && typeof this._settings.transform !== 'function';

        if (isInvalidTransform) {
            throw new Error('settings.transform must be a function.');
        }
    }
     ...
     *
     * @returns {{isTar: boolean, isGzip: boolean, gzipOptions: Object, dotFiles: boolean, emptyFiles: boolean, emptyDirs: boolean, transform: Function }}
     */
    get settings() {
