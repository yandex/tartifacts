...
const VinylConvertStream = require('../streams/vinyl-convert-stream');
...
const writeArtifact = (artifactTask) => {
    ...
    const vinylConvertStream = new VinylConvertStream();

    return new Promise((resolve, reject) => {
        vinylConvertStream.on('error', reject);
        readStream.on('error', reject);
        writeStream.on('error', reject).on('close', resolve);

        const transformedStream = artifactSettings.transformStreams.reduce((prevStream, transformStream) => {
            return prevStream.pipe(transformStream).on('error', reject);
        }, readStream.pipe(vinylConvertStream));
 ...
 * @param {stream.Transform[]} [options.transformStreams] The array of transform streams.
 * @returns {Promise}
 */
